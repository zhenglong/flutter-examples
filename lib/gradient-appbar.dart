...
class GradientAppBarState extends State<GradientAppBar> {
  ...
  Widget build(BuildContext context) {
    ...
    TextStyle centerStyle = widget.textTheme?.title ?? appBarTheme.textTheme?.title ?? themeData.primaryTextTheme.title;
    TextStyle sideStyle = widget.textTheme?.body1 ?? appBarTheme.textTheme?.body1 ?? themeData.primaryTextTheme.body1;

    Widget leading = widget.leading;
    if (leading == null && widget.automaticallyImplyLeading) {
      if (hasDrawer) {
        leading = IconButton(icon: const Icon(Icons.menu), onPressed: _handleDrawerButton);
      } else {
        if (canPop) {
    ...
    return Material(
      color: widget.gradientStart != null && widget.gradientEnd != null ? Colors.transparent : widget.backgroundColor ?? appBarTheme.color ?? themeData.primaryColor,
